...
{
    string normalized = webLogPathCompat.Trim();
    string? compatDir = Path.GetDirectoryName(normalized);
    if (!string.IsNullOrWhiteSpace(compatDir))
    {
        webLogDir = compatDir;
    }
    string compatPrefix = Path.GetFileNameWithoutExtension(normalized);
    if (!string.IsNullOrWhiteSpace(compatPrefix))
    {
...
{
    int port = 8085;
    if (Uri.TryCreate(webClientUrl,
            UriKind.Absolute,
            out var parsed) && parsed.Port > 0)
    {
        port = parsed.Port;
    }
    else if (int.TryParse(Environment.GetEnvironmentVariable("HIDBRIDGE_WEB_PORT"),
                 NumberStyles.Integer,
                 CultureInfo.InvariantCulture,
                 out int envPort) && envPort > 0)
    {
        port = envPort;
    }
...
{
    builder.WebHost.UseUrls(webClientUrl,
        webClientHttpsUrl);
}
...
static Uri ToWsUri(Uri httpBase, string path)
{
    ...
    b.Scheme = b.Scheme.Equals("https",
        StringComparison.OrdinalIgnoreCase) ? "wss" : "ws";
...
{
    Uri wsUri = ToWsUri(serverHttpBase,
        "/ws/hid");
    await using var ws = new HidControlWsClient();
    if (!string.IsNullOrWhiteSpace(token))
    {
        ws.SetRequestHeader("X-HID-Token",
            token!);
    }
...
app.Map("/ws/webrtc",
    async ctx =>
    {
        ...
        Uri serverWsUri = ToWsUri(serverHttpBase,
            "/ws/webrtc");

        using var serverWs = new ClientWebSocket();
        if (!string.IsNullOrWhiteSpace(token))
        {
            serverWs.Options.SetRequestHeader("X-HID-Token",
                token!);
        }

        await serverWs.ConnectAsync(serverWsUri,
            ctx.RequestAborted);

        var toServer = PumpAsync(browserWs,
            serverWs,
            ctx.RequestAborted);
        var toBrowser = PumpAsync(serverWs,
            browserWs,
            ctx.RequestAborted);

        await Task.WhenAny(toServer,
            toBrowser);
        try
        {
        ...
        try
        {
            await serverWs.CloseAsync(WebSocketCloseStatus.NormalClosure,
                "closing",
                CancellationToken.None);
        }
        catch
                                        ...
                                        if (body) {
                                          ...
                                          body.innerHTML = `<tr>
                                              ...
                                              <td style="padding: 6px 4px">
                                                ...
                                                <button data-act="restart" data-room="${r}">Restart</button>
                                                <button data-act="use" data-room="${r}">Use</button>
                                                <button data-act="connect" data-room="${r}">Connect</button>
                                                <button data-act="hangup" data-room="${r}" disabled>Hangup</button>
                                      ...
                                      try {
                                        ...
                                        if (!ok && roomsRaw.length === 0) {
                                          ...
                                          rtcLog({
                                            ...
                                            mode,
                                            count: body ? body.children.length : 0,
                                            reason: "payload_not_ok",
                                            keys: j && typeof j === "object" ? Object.keys(j).slice(0, 8) : []
                                          });
                                          return;
                                        }
...
app.MapPost("/api/shortcut",
    async (KeyboardShortcutRequest req,
        CancellationToken ct) =>
    {
        ...
        string resp = await SendWsOnceAsync(baseUri,
            token,
            (ws,
                    c) =>
                ws.SendKeyboardShortcutAsync(req.Shortcut,
                    itfSel: req.ItfSel,
                    holdMs: req.HoldMs ?? 80,
                    applyMapping: req.ApplyMapping ?? true,
                    id: Guid.NewGuid()
                        .ToString("N"),
                    ct: c),
            ct);
    ...
    async (CancellationToken ct) =>
    {
        try
        {
            ...
            var client = new HidControl.ClientSdk.HidControlClient(http,
                new Uri(serverUrl));
            var res = await HidControl.ClientSdk.WebRtcClientExtensions.GetWebRtcIceAsync(client,
                ct);
            return Results.Json(res ??
                                new HidControl.Contracts.WebRtcIceResponse(false,
                                    null,
...
app.MapGet("/api/webrtc/config",
    async (CancellationToken ct) =>
    {
        try
        {
            ...
            var client = new HidControl.ClientSdk.HidControlClient(http,
                new Uri(serverUrl));
            var res = await HidControl.ClientSdk.WebRtcClientExtensions.GetWebRtcConfigAsync(client,
                ct);
            return Results.Json(res ??
                                new HidControl.Contracts.WebRtcConfigResponse(false,
                                    0,
                                    0,
                                    0,
                                    0,
...
app.MapGet("/api/webrtc/rooms",
    async (CancellationToken ct) =>
    {
        static string NormalizeRoomsPayload(string? body,
            bool forceOk,
            string? error = null)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(body))
                {
                    ...
                    if (doc.RootElement.ValueKind == JsonValueKind.Object)
                    {
                        ...
                        bool hasRooms = root.TryGetProperty("rooms",
                                            out var roomsEl) &&
                                        roomsEl.ValueKind == JsonValueKind.Array;
                        bool hasOk = root.TryGetProperty("ok",
                            out _);
                        if (hasRooms && hasOk && !forceOk)
                            return body!;
                        if (hasRooms)
                        {
                            var rooms = root.GetProperty("rooms")
                                            .Deserialize<object[]>() ??
                                        Array.Empty<object>();
        ...
        try
        {
            ...
            string normalized = NormalizeRoomsPayload(body,
                forceOk: !resp.IsSuccessStatusCode,
                error: resp.IsSuccessStatusCode
                    ? null
                    : $"http_{(int)resp.StatusCode}");
            return Results.Content(normalized,
                "application/json",
                statusCode: StatusCodes.Status200OK);
        }
        catch (TaskCanceledException)
        {
            return Results.Content(NormalizeRoomsPayload(null,
                    forceOk: true,
                    error: "timeout"),
                "application/json",
                statusCode: StatusCodes.Status200OK);
        ...
        CancellationToken ct) =>
    {
        try
        {
            ...
            var client = new HidControl.ClientSdk.HidControlClient(http,
                new Uri(serverUrl));
            var body =
                await req.ReadFromJsonAsync<HidControl.Contracts.WebRtcCreateRoomRequest>(cancellationToken: ct) ??
                new HidControl.Contracts.WebRtcCreateRoomRequest(null);
            var res = await HidControl.ClientSdk.WebRtcClientExtensions.CreateWebRtcRoomAsync(client,
                body.Room,
                ct);
            return Results.Json(res ??
...
app.MapDelete("/api/webrtc/rooms/{room}",
    async (string room,
        CancellationToken ct) =>
    {
        try
        {
            ...
            var client = new HidControl.ClientSdk.HidControlClient(http,
                new Uri(serverUrl));
            var res = await HidControl.ClientSdk.WebRtcClientExtensions.DeleteWebRtcRoomAsync(client,
                room,
                ct);
            return Results.Json(res ??
                                new HidControl.Contracts.WebRtcDeleteRoomResponse(false,
                                    room,
                                    false,
                                    "delete_failed"));
        }
        ...
        {
            return Results.Json(new HidControl.Contracts.WebRtcDeleteRoomResponse(false,
                room,
                false,
                "timeout"));
        }
...
app.MapGet("/api/webrtc/video/rooms",
    async (CancellationToken ct) =>
    {
        try
        {
...
app.MapGet("/api/webrtc/video/peers/{room}",
    async (string room,
        CancellationToken ct) =>
    {
        try
...
app.MapGet("/api/video/dshow/modes",
    async (string? device,
        CancellationToken ct) =>
    {
...
app.MapGet("/api/video/webrtc/encoders",
    async (CancellationToken ct) =>
    {
        try
        {
...
app.MapPost("/api/webrtc/video/rooms",
    async (HttpRequest req,
        CancellationToken ct) =>
    {
        ...
        catch (TaskCanceledException)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(token))
                    probe.DefaultRequestHeaders.Add("X-HID-Token",
                        token);
                var client = new HidControl.ClientSdk.HidControlClient(probe,
                    new Uri(serverUrl));
                var list = await HidControl.ClientSdk.WebRtcClientExtensions.ListWebRtcVideoRoomsAsync(client,
                    CancellationToken.None);
                var wanted = body.Room;
                if (!string.IsNullOrWhiteSpace(wanted))
                {
                    bool exists = list?.Rooms?.Any(r => string.Equals(r.Room,
                                      wanted,
                ...
                {
                    string statusUrl =
                        $"{serverUrl.TrimEnd('/')}/status/webrtc/video/peers/{Uri.EscapeDataString(wanted)}";
                    using var statusResp = await probe.GetAsync(statusUrl,
                        CancellationToken.None);
                    if (statusResp.IsSuccessStatusCode)
                    {
                        using var doc =
                            JsonDocument.Parse(await statusResp.Content.ReadAsStringAsync(CancellationToken.None));
                        bool running = doc.RootElement.TryGetProperty("ok",
                                           out var okEl) &&
                                       okEl.GetBoolean() &&
                                       doc.RootElement.TryGetProperty("running",
                                           out var runEl) &&
                                       runEl.GetBoolean();
                        if (running)
                        {
...
app.MapPost("/api/webrtc/video/rooms/{room}/apply",
    async (string room,
        HttpRequest req,
        CancellationToken ct) =>
    {
        var payload = new HidControl.Contracts.WebRtcCreateVideoRoomRequest(room,
            null,
            null,
            null,
            null,
        ...
        try
        {
            payload =
                await req.ReadFromJsonAsync<HidControl.Contracts.WebRtcCreateVideoRoomRequest>(cancellationToken: ct) ??
                payload;
            string targetRoom = string.IsNullOrWhiteSpace(payload.Room)
                ? room
                : payload.Room.Trim();
            ...
            try
            {
                deleted = await HidControl.ClientSdk.WebRtcClientExtensions.DeleteWebRtcVideoRoomAsync(client,
                    targetRoom,
                    ct);
            }
            catch
...
app.MapPost("/api/webrtc/video/rooms/{room}/restart",
    async (string room,
        HttpRequest req,
        CancellationToken ct) =>
    {
        ...
        try
        {
            ...
            string url =
                $"{serverUrl.TrimEnd('/')}/status/webrtc/video/rooms/{Uri.EscapeDataString(targetRoom)}/restart";
...
app.MapDelete("/api/webrtc/video/rooms/{room}",
    async (string room,
        CancellationToken ct) =>
    {
        try
        {
            ...
            var client = new HidControl.ClientSdk.HidControlClient(http,
                new Uri(serverUrl));
            var res = await HidControl.ClientSdk.WebRtcClientExtensions.DeleteWebRtcVideoRoomAsync(client,
                room,
                ct);
            return Results.Json(res ??
                                new HidControl.Contracts.WebRtcDeleteRoomResponse(false,
        ...
        catch (TaskCanceledException)
        {
            try
            {
                ...
                var list = await HidControl.ClientSdk.WebRtcClientExtensions.ListWebRtcVideoRoomsAsync(client,
                    ct);
                bool stillExists = list?.Rooms?.Any(r => string.Equals(r.Room,
                                       room,
                                       StringComparison.OrdinalIgnoreCase)) ==
                                   true;
                if (!stillExists)
                {
                    return Results.Json(new HidControl.Contracts.WebRtcDeleteRoomResponse(true,
                        room,
                        true,
                        null));
                }
...
app.MapPost("/api/keyboard/text",
    async (KeyboardTextRequest req,
        CancellationToken ct) =>
    {
        Uri baseUri = new Uri(serverUrl);
        string resp = await SendWsOnceAsync(baseUri,
            ...
            (ws,
                    c) =>
                ws.SendKeyboardTextAsync(req.Text ?? "",
                    layout: req.Layout,
                    itfSel: req.ItfSel,
                    id: Guid.NewGuid()
                        .ToString("N"),
...
app.MapPost("/api/keyboard/press",
    async (KeyboardPressApiRequest req,
        CancellationToken ct) =>
    {
        if (!TryParseByte(req.Usage,
                out byte usage))
        {
            return Results.BadRequest(new
            {
                ok = false,
                error = "usage must be 0..255 (decimal) or 0x.."
            });
        }

        if (!TryParseByte(req.Mods,
                out byte mods))
        {
            return Results.BadRequest(new
            {
                ok = false,
                error = "mods must be 0..255 (decimal) or 0x.."
            });
        }
        ...
        string resp = await SendWsOnceAsync(baseUri,
            ...
            (ws,
                    c) =>
                ws.SendKeyboardPressAsync(usage,
                    mods: mods,
                    itfSel: req.ItfSel,
                    id: Guid.NewGuid()
                        .ToString("N"),
...
app.MapPost("/api/keyboard/chord",
    async (KeyboardChordRequest req,
        CancellationToken ct) =>
    {
        ...
        foreach (int k in keys)
        {
            if (k < 0 || k > 255)
            {
                ...
                {
                    ok = false,
                    error = "keys must be 0..255"
                });
            }
        }

        Uri baseUri = new Uri(serverUrl);
        Uri wsUri = ToWsUri(baseUri,
            "/ws/hid");
        ...
        var keyBytes = keys.Select(k => (byte)k)
            .ToArray();
        string downId = Guid.NewGuid()
            .ToString("N");
        await ws.SendKeyboardReportAsync(mods,
            keyBytes,
            applyMapping: req.ApplyMapping ?? true,
            itfSel: req.ItfSel,
            id: downId,
        ...
        string upId = Guid.NewGuid()
            .ToString("N");
        await ws.SendKeyboardReportAsync(0,
            Array.Empty<byte>(),
            applyMapping: req.ApplyMapping ?? true,
            itfSel: req.ItfSel,
            id: upId,
...
app.MapPost("/api/keyboard/report",
    async (KeyboardReportApiRequest req,
        CancellationToken ct) =>
    {
        byte mods = req.Mods ?? 0;
        int[] keys = req.Keys ?? Array.Empty<int>();
        ...
        string resp = await SendWsOnceAsync(baseUri,
            ...
            (ws,
                    c) =>
                ws.SendKeyboardReportAsync(mods,
                    keys.Select(x => (byte)x)
                        .ToArray(),
                    applyMapping: req.ApplyMapping ?? true,
                    itfSel: req.ItfSel,
                    id: Guid.NewGuid()
...
app.MapPost("/api/keyboard/down",
        ...
        CancellationToken ct) =>
    {
        ...
        string resp = await SendWsOnceAsync(baseUri,
            ...
            (ws,
                    c) =>
                ws.SendKeyboardDownAsync(usage,
                    mods: mods,
                    itfSel: req.ItfSel,
                    id: Guid.NewGuid()
                        .ToString("N"),
...
app.MapPost("/api/mouse/move",
    async (MouseMoveApiRequest req,
        CancellationToken ct) =>
    {
        Uri baseUri = new Uri(serverUrl);
        string resp = await SendWsOnceAsync(baseUri,
            ...
            (ws,
                    c) =>
                ws.SendMouseMoveAsync(req.Dx ?? 0,
                    req.Dy ?? 0,
                    itfSel: req.ItfSel,
                    id: Guid.NewGuid()
                        .ToString("N"),
...
app.MapPost("/api/mouse/click",
    async (MouseClickApiRequest req,
        CancellationToken ct) =>
    {
        string button = string.IsNullOrWhiteSpace(req.Button)
            ? "left"
            : req.Button!;
        Uri baseUri = new Uri(serverUrl);
        Uri wsUri = ToWsUri(baseUri,
        ...
        await ws.SendMouseButtonAsync(button,
            down: true,
            itfSel: req.ItfSel,
            id: Guid.NewGuid()
                .ToString("N"),
...
app.MapPost("/api/mouse/button",
    async (MouseButtonStateApiRequest req,
        CancellationToken ct) =>
    {
        string button = string.IsNullOrWhiteSpace(req.Button)
            ? "left"
        ...
        string resp = await SendWsOnceAsync(baseUri,
            ...
            (ws,
                    c) =>
                ws.SendMouseButtonAsync(button,
                    down,
                    itfSel: req.ItfSel,
                    id: Guid.NewGuid()
                        .ToString("N"),
...
app.MapPost("/api/mouse/wheel",
    async (MouseWheelApiRequest req,
        CancellationToken ct) =>
    {
        ...
        await ws.SendJsonAsync(payload,
            null,
            ct);
...
app.MapGet("/sdk/ping",
    () =>
    {
        _ = new HidControlClient(new HttpClient(),
...
static bool TryParseByte(string? s, out byte value)
{
    ...
    if (t.StartsWith("0x",
            StringComparison.OrdinalIgnoreCase))
    {
        return byte.TryParse(t.AsSpan(2),
            System.Globalization.NumberStyles.HexNumber,
            System.Globalization.CultureInfo.InvariantCulture,
            out value);
    }
    return byte.TryParse(t,
...
static async Task PumpAsync(WebSocket src, WebSocket dst, CancellationToken ct)
{
    ...
    while (!ct.IsCancellationRequested && src.State == WebSocketState.Open && dst.State == WebSocketState.Open)
    {
        ...
        try
        {
            result = await src.ReceiveAsync(buffer,
                ct);
        }
        catch
        {
        ...
        if (result.Count > 0)
        {
            await dst.SendAsync(new ArraySegment<byte>(buffer,
                    0,
                    result.Count),
                result.MessageType,
                result.EndOfMessage,
